...
I already uploaded this data to my github, let's load it into RStudio.
...
### Query data to return only females
...
Let's create the string and query the data between June 1, 2020 and the present. 
I used a trick for the present date, which is to grab the computer's system date (Sys.Date()) which should be today's date.
...
neigh_name <- as.data.frame(table(lacity_data$area_name))
colnames(neigh_name) <- c("Name", "Count")
wordcloud(neigh_name$Name, neigh_name$Count, min.freq = 100, max.words = 15, random.order = T, random.color = T, colors =c("green", "cornflowerblue", "darkred"), scale = c(2,.3))
```
...
tbl_lookup<-read.csv("/Volumes/Seagate Backup Plus Drive/Barboza Law/COC presentation/Day 2/MO_CODES_Numerical_20180627.csv")
...
R has a number of powerful GIS-related functions. We will use a number of these functions to map our data. R makes it possible to read and edit spatial data, conduct geoprocessing and spatial analysis and create static and interactive maps. 

In order to map the boundary of a region, we need to have a shapefile. There are many types of shapefiles: point, line and polygon.

I have already downloaded a shapefile of the city of LA, and it also contains data on COVID-19 cases across the city.

The first step in spatial analysis is reading in your spatial data. There are many ways to do this. The first is via **rgdal** 

** readOGR(dsn="PATH TO FILE", layer="ACTUAL FILE (WITH NO EXTENSION)")

For example, if this is my path: "D:/Barboza Law/COC presentation/Day 2/shapefiles/covid-19 by neighborhood/nghbrhd_data.shp"

This is how you read it into R:
** readOGR(dsn="D:/Barboza Law/COC presentation/Day 2/shapefiles/covid-19 by neighborhood", layer="nghbrhd_data")

Notice, these won't work:
** readOGR(dsn="D:/Barboza Law/COC presentation/Day 2/shapefiles/covid-19 by neighborhood/", layer="nghbrhd_data")
** readOGR(dsn="D:/Barboza Law/COC presentation/Day 2/shapefiles/covid-19 by neighborhood/", layer="nghbrhd_data.shp")

I use the sf package, I find it easier and it is more the more currently accepted way. An example is below.
...
```{r open shapefile}
lacity_bdry <- st_read("/Volumes/Seagate Backup Plus Drive/Barboza Law/COC presentation/Day 2/shapefiles/covid-19 by neighborhood/nghbrhd_data.shp") %>%
  st_transform(4269)

plot(lacity_bdry)
```

Recall that we discussed last week that the census tracts can be represented as a network... 
Below I am going to use what is called a queen contiguity scheme to plot the nearest neighbor census tracts in a network style graph.

```{r la as network}
#if FALSE, two units are neighbors only if they share a corner and edge (not just an edge)
continuity.nb <- poly2nb(lacity_bdry, queen = TRUE) 
# shows how many links each polygon (i.e. CT) has, mean connections=5.2
summary(continuity.nb) # Average number of links is 4.283465 
lacity_bdry_SP <- as(lacity_bdry, "Spatial") # from sf to sp
plot(continuity.nb, coordinates(lacity_bdry_SP))

```

We will get back to this idea of a spatial network below.

In order to map the IPV incidents involving homosexual victims onto the city boundary, it must be transformed it into a map file, which is really easy using the **sf** library in R. Here are the simple steps:
...
Note: a coordinate reference system (CRS) refers to the way in which spatial data are flattened so that the points can be drawn on a 2-dimensional surface
...
A chloropleth map uses different shades, colors and symbols within predefined areas (e.g census tracts, neighborhoods) to indicate the summary value (e.g average, count, percent) of a property (income, population, crimes) in those areas.

To make a chloropleth map of the crime incidents using the LA neighborhood file, we need to first count the number of points in each neighborhood as follows

```{r}
merged_crimes <- st_join(lacity_bdry,points_sf) %>% 
  dplyr::group_by(COMTY_NAME) %>%                  # group by unique neighborhood
...
```{r prepare dat}
library(data.table)
#Load data and create an ID variable
...
child_deaths$ID <- paste0("V", 1:nrow(child_deaths))
rownames(child_deaths) <- child_deaths$ID

#rename variables
child_deaths <- plyr::rename(child_deaths, c("x"="lon", "y"="lat"))

#remove empty lat/long locations
child_deaths <- child_deaths[!is.na(child_deaths$lon), ]

#select lat long only and store into dataframe called 'coords'
coords = cbind(child_deaths$lon, child_deaths$lat)
```

```{r}
library(spdep)

# spDists returns a full matrix of distances in the metric of the points if longlat=FALSE, or in kilometers if longlat=TRUE;
distance <- spDists(coords, coords, longlat=T)

# scale the distance so it returns miles
distance <- distance * 0.621371192237

# this makes a matrix of nearest neighbors for each point in the data set
deaths.nb<-dnearneigh(coords, 0, 5.60934, row.names = child_deaths$ID, longlat = TRUE)

#The nb2listw function supplements a neighbours list with spatial weights for the chosen coding scheme.
#Here we weight by how many nearest neighbors the point has
deaths.listw<-nb2listw(deaths.nb, zero.policy=TRUE)
(w1 = listw2mat(deaths.listw))

```
```{r datacheck}
data.frame(deaths.listw$weights[[1]])
data.frame(deaths.listw$neighbours[[1]])
(w_p1 <- 1/29)
```

```{r neighbor to dataframe}
...
nb_df <- nb_to_df(deaths.listw$neighbours, coords)
...
```{r add attribute}
table(child_deaths$causes)
coord_df$causes<-child_deaths$causes
coord_df$causes[coord_df$causes=="Gunshot"]<-"Gunshot"
coord_df$causes[coord_df$causes=="Blunt force"]<-"Blunt force"
coord_df$causes[coord_df$causes=="Stabbing"]<-"Stabbing"
coord_df$causes[coord_df$causes=="Other" | coord_df$causes=="Strangled" | coord_df$causes=="Unknown" ]<-NA
table(coord_df$causes)
coord_df <- coord_df[!is.na(coord_df$causes), ]
```

```{r}
library(ggmap)
#Get coordinates from https://www.openstreetmap.org/#map=4/38.01/-95.84
bbox <- c(left=-118.6681776, bottom=33.659541, right=  -118.1552947, top=34.337306)

#bounding box of southwest and northeast coordinates
la <- get_stamenmap(bbox, zoom = 12, maptype = "toner-lite")

```

```{r}
ggmap(la, extent = 'device', darken = c(.01, "black")) +
  theme(
    legend.key.size = grid::unit(1.8,"lines"),
    legend.title = element_text(size = 16, face = "bold"),
    legend.text = element_text(size = 14)) + geom_segment(aes(x=x, xend=xend, y=y, yend=yend), colour="red", size =
                   .5, data=nb_df) +
  geom_point(aes(x=lon, y=lat, colour = causes), data=coord_df, size=2.0) +
  scale_color_manual(values=c("#56B4E9", "#D55E00", "#0000FF"))+
  ylab("Latitude") +
  xlab("Longitude") +
  labs(caption = "Map created using data from the Los Angeles Times Homicide Report") +
  ggtitle("Identification 3-mile Radius Neighbor Specification")

```

```{r}
#Our old friend from last week, igraph
library(igraph)
w1[w1>0]<-1
g1<-graph.adjacency(w1)

#layout representing actual space
plot(g1, vertex.color=ifelse(child_deaths$causes == "Gunshot", "grey10",ifelse(child_deaths$causes == "Blunt force" , "grey47", "grey74")),
     vertex.label=child_deaths$name, layout=coords, vertex.size=2.5,
     vertex.label.color="grey23", edge.arrow.size=.1, edge.color="red", vertex.label.cex=.5)

#A different layout
plot(g1, edge.arrow.size=.2, edge.color="red",
     vertex.color="orange", vertex.frame.color="#ffffff",vertex.size=2.5,
     vertex.label=child_deaths$name, vertex.label.color="navy", vertex.label.cex=.5) 
```
```{r}
###K-neighbor approach
deaths.nb<-knn2nb(knearneigh(coords,k=1))
deaths.listw1c<-nb2listw(deaths.nb)

w1c = listw2mat(deaths.listw1c)

w1c
nb_df <- nb_to_df(deaths.listw1c$neighbours, coords)
```
...
ggmap(la, extent = 'device', darken = c(.01, "black")) +
  theme(
    legend.key.size = grid::unit(1.8,"lines"),
    legend.title = element_text(size = 16, face = "bold"),
    legend.text = element_text(size = 14)) + geom_segment(aes(x=x, xend=xend, y=y, yend=yend), colour="red", size = .5, data=nb_df) + 
  geom_segment(aes(x=x, xend=xend, y=y, yend=yend), colour="red", size = .8, data=nb_df) +
  geom_point(aes(x=lon, y=lat, colour = causes), data=coord_df, size=1.8) +
  scale_color_manual(values=c("#56B4E9", "#D55E00", "#0000FF"))+
  ylab("Latitude") +
  xlab("Longitude") +
  labs(caption = "Map created using data from the Los Angeles Times Homicide Report") +
  ggtitle("Identification with Closest Neighbor Specification")
```

```{r}
#Network representation
w1c[w1c>0]<-1
g1c<-graph.adjacency(w1c)
#Sociogram shown in Figure 4(d)

plot(g1c, vertex.color=ifelse(child_deaths$causes =="Gunshot", "red", "green"),
     vertex.label= NA, layout=coords, vertex.size=1.8,
     vertex.label.color="grey23", edge.arrow.size=.2, edge.color="grey23")
...
From the file menu File --> New File --> R Markdown